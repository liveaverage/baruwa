...
urlpatterns = patterns('baruwa.messages.views',
    ...
    (r'^(?P<view_type>(full|quarantine))/$', 'index', {'list_all': 1}, 'all-messages-index'),
    (r'^(?P<view_type>(full|quarantine))/(?P<direction>(dsc|asc))/(?P<order_by>(timestamp|from_address|to_address|subject|size|sascore))/$',
    'index', {'list_all': 1}, 'all-messages-list'),
    (r'^(?P<view_type>(full|quarantine))/(?P<page>([0-9]+|last))/$', 'index', {'list_all': 1}, 'all-messages-page'),
    (r'^(?P<view_type>(full|quarantine))/(?P<page>([0-9]+|last))/(?P<direction>(dsc|asc))/(?P<order_by>(timestamp|from_address|to_address|subject|size|sascore))/$',
    'index', {'list_all': 1}, 'sorted-messages-page'),
    (r'^(?P<view_type>(full|quarantine))/(?P<quarantine_type>(spam|policyblocked))/(?P<direction>(dsc|asc))/(?P<order_by>(timestamp|from_address|to_address|subject|size|sascore))/$',
