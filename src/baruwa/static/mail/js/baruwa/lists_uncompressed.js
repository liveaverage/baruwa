...
function ajax_error(event, request, settings){
    if(request.status == 200){
        if(settings.url == '/mail/lists/add/'){
...
function toplinkize(list_kind,direction,field_name){
    var tmp = '';
    if(direction == 'dsc'){
        tmp = ' <a href="/mail/lists/'+list_kind+'/asc/'+field_name+'/">&uarr;</a>';
    }else{
        tmp = ' <a href="/mail/lists/'+list_kind+'/dsc/'+field_name+'/">&darr;</a>';
    }
    return tmp;
}
...
function paginate(){
    ...
    if(rj.show_first){
        tmp +='<span><a href="/mail/lists/'+rj.list_kind+'/1/'+rj.direction+'/'+rj.order_by+'/"><img src="'+media_url+'common/imgs/first_pager.png" alt="First"/></a></span>';
        tmp +='<span>.....</span>';
    }
    if(rj.has_previous){
        tmp +='<span><a href="/mail/lists/'+rj.list_kind+'/'+rj.previous+'/'+rj.direction+'/'+rj.order_by+'/"><img src="'+media_url+'common/imgs/previous_pager.png" alt="Previous"/></a></span>';
    }
    $.each(rj.page_numbers,function(itr,lnk){
        li = '/mail/lists/'+rj.list_kind+'/'+lnk+'/'+rj.direction+'/'+rj.order_by+'/';
        if(rj.page == lnk){
    ...
    if(rj.has_next){
        tmp +='<span><a href="/mail/lists/'+rj.list_kind+'/'+rj.next+'/'+rj.direction+'/'+rj.order_by+'/"><img src="'+media_url+'common/imgs/next_pager.png" alt="Next"/></a></span>';
    }
    if(rj.show_last){
        tmp +='<span>......</span>';
        tmp +='<a href="/mail/lists/'+rj.list_kind+'/last/'+rj.direction+'/'+rj.order_by+'/"><img src="'+media_url+'common/imgs/last_pager.png" alt="Last"/></a></span>';
    }
    ...
    for(i=0; i< carray.length;i++){
        ...
        if(rj.order_by == carray[i]){
            ...
            $('#lists_filter_form').attr('action','/mail/lists/'+rj.list_kind+'/'+rj.direction+'/'+carray[i]+'/');
        }else{
            ur = '<a href="/mail/lists/'+rj.list_kind+'/'+rj.direction+'/'+carray[i]+'/">'+gettext(h)+'</a>';
...
function lists_from_json(data){
    if(data){
        ...
        $.each(data.items,function(i,n){
            ...
            link = '<a href="/mail/lists/delete/'+n.id+'/"><img src="'+media_url+'common/imgs/action_delete.png" title="Delete" alt="Delete" /></a>';
...
function handlextern(){
    ...
    if(page){
        window.scrollTo(0,0);
        page = $.trim(page);
        re = /^mail\-lists\-[1-2]\-[0-9]+\-dsc|asc\-id|to_address|from_address$/
        if(re.test(page)){
            re = /^mail\-lists\-([1-2])\-[0-9]+\-dsc|asc\-id|to_address|from_address$/
            f = page.match(re);
            page = page.replace(/-/g,'/');
