function handle_listing(event){
    ...
    $('#yes_list').bind('click', function(event) {
        ...
        ajax_target = 2;
        $.post(lists_add_url, list_params, function(response, textStatus, xhr) {
            if (response.success) {
...
$(document).ready(function() {
    ...
    mh = $('#mail-headers');
    mh.hide();
    mh.after($("<a/>").attr({href:'#',id:'header-toggle',innerHTML:'<img src="'+media_url+'mail/imgs/maximize.png" alt="&darr;">&nbsp;'+gettext('Show headers')}));
    $("#header-toggle").bind('click',function(event){
        event.preventDefault();
        if($("#mail-headers").css("display") == 'block'){
            $("#mail-headers").css({display:'none'})
            $(this).blur().html('<img src="'+media_url+'mail/imgs/maximize.png" alt="&darr;">&nbsp;'+gettext('Show headers'));
            window.scroll(0,50);
        }else{
            $("#mail-headers").css({display:'block'})
            $(this).blur().html('<img src="'+media_url+'mail/imgs/minimize.png" alt="&uarr;">&nbsp;'+gettext('Hide headers'));
        }
    });
