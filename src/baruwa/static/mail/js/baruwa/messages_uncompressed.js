...
function toplinkize(direction,view_type,field_name){
    var tmp = '';
    if(direction == 'dsc'){
        tmp = ' <a href="'+messages_base_url+view_type+'/asc/'+field_name+'/">&uarr;</a>';
    }else{
        tmp = ' <a href="'+messages_base_url+view_type+'/dsc/'+field_name+'/">&darr;</a>';
    }
    return tmp;
}
...
function handlextern(){
   ...
   if(page){
        page = $.trim(page);
        re = /^mail\-messages\-full|archive\-[0-9]+|last\-dsc|asc\-timestamp|to_address|from_address|subject|size|sascore$/;
        if(re.test(page)){
            page = page.replace(/-/g,'/');
            url = '/'+ page + '/';
            window.scrollTo(0,0);
...
function paginate(){
   ...
   if(rj.show_first){
        if(rj.direction){
            li=messages_base_url+rj.view_type+'/'+rj.direction+'/'+rj.order_by+'/';
        }else{
            li=messages_base_url+rj.view_type+'/'+rj.order_by+'/';
        }
   ...
   if(rj.has_previous){
        if(rj.direction){
            li=messages_base_url+rj.view_type+'/'+rj.previous+'/'+rj.direction+'/'+rj.order_by+'/';
        }else{
            li=messages_base_url+rj.view_type+'/'+rj.previous+'/'+rj.order_by+'/';
        }
   ...
   $.each(rj.page_numbers,function(itr,lnk){
        if(rj.page == lnk){ 
            ...
            if(rj.direction){
                li=messages_base_url+rj.view_type+'/'+lnk+'/'+rj.direction+'/'+rj.order_by+'/';
            }else{
                li=messages_base_url+rj.view_type+'/'+lnk+'/'+rj.order_by+'/';
            }
   ...
   if(rj.has_next){
        if(rj.direction){
            li=messages_base_url+rj.view_type+'/'+rj.next+'/'+rj.direction+'/'+rj.order_by+'/';
        }else{
            li=messages_base_url+rj.view_type+'/'+rj.next+'/'+rj.order_by+'/';
        }
        tmp +='<span><a href="'+li+'"><img src="'+media_url+'mail/imgs/next_pager.png" alt="Next"/></a></span>';
   }
   if(rj.show_last){
        if(rj.direction){
            li=messages_base_url+rj.view_type+'/last/'+rj.direction+'/'+rj.order_by+'/';
        }else{
            li=messages_base_url+rj.view_type+'/last/'+rj.order_by+'/';
        }
        tmp +='<span>......</span>';
    ...
    for(i=0; i< carray.length;i++){
        ...
        if(carray[i] == rj.order_by){
            ...
            ur = '<a href="'+messages_base_url+rj.view_type+'/'+rj.direction+'/'+carray[i]+'/">'+h+'</a>';
