...
function paginate(){
    ...
    if(rj.show_first){
       ...
       row[count++] = '<img src="'+media_url+'mail/imgs/first_pager.png" alt="First"/></a></span>';
       row[count++] = '<span>.....</span>';
    }
    if(rj.has_previous){
       row[count++] = '<span><a href="/'+rj.app+'/'+rj.previous+'/'+rj.direction+'/'+rj.order_by+'/">';
       row[count++] = '<img src="'+media_url+'mail/imgs/previous_pager.png" alt="Previous"/></a></span>';
    }
    ...
    if(rj.has_next){
       ...
       row[count++] = '<img src="'+media_url+'mail/imgs/next_pager.png" alt="Next"/></a></span>';
    }
    if(rj.show_last){
       row[count++] = '<span>......</span>';
       row[count++] = '<a href="/'+rj.app+'/last/'+rj.direction+'/'+rj.order_by+'/">';
       row[count++] = '<img src="'+media_url+'mail/imgs/last_pager.png" alt="Last"/></a></span>';
    }
...
function page_from_json(data){
    if (data) {
        ...
        $.each(data.items, function(i, n) {
            ...
            if (n.enabled) {
                img = '<img src="'+media_url+'mail/imgs/active.png" alt="" />';
                eimg = '<img src="'+media_url+'mail/imgs/tick.png" alt="" />';
            }else{
                img = '<img src="'+media_url+'mail/imgs/inactive.png" alt="" />';
                eimg = '<img src="'+media_url+'mail/imgs/minus.png" alt="" />';
            };
            ...
            row[count++] = '<img src="'+media_url+'mail/imgs/edit.png" alt="Edit" title="Edit" /></a></div>';
            row[count++] = '<div class="Domains_action_delete">';
            row[count++] = '<a href="/accounts/delete/address/'+n.id+'/">';
            row[count++] = '<img src="'+media_url+'mail/imgs/action_delete.png" alt="Delete" title="Delete" /></a>';
...
function handlextern(){
    ...
    if(page){
        window.scrollTo(0,0);
        page = $.trim(page);
        re = /^mail-settings\-[0-9]+\-dsc|asc\-id|address$/
        if(re.test(page)){
            page = page.replace(/-/g,'/');
            url = '/'+ page + '/';
