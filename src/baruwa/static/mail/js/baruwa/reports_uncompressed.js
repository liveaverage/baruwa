...
function build_active_filters(active_filters){
    ...
    $.each(active_filters,function(itr,filter){
        ...
        rows[count++] = '<a href="/mail/reports/fd/'+itr+'/"><img src="'+media_url+'mail/imgs/action_remove.png" alt="x" title="Remove" /></a>';
        rows[count++] = '</div>';
        rows[count++] = '<div class="Filter_save">';
        rows[count++] = '<a href="/mail/reports/fs/'+itr+'/"><img src="'+media_url+'mail/imgs/save.png" alt="Save" title="Save" /></a>';
        rows[count++] = '</div>';
...
function build_saved_filters(saved_filters){
    ...
    $.each(saved_filters,function(itr,filter){
        ...
        rows[count++] = '<div class="Active_filters">';
        rows[count++] = '<div class="Filter_remove">';
        rows[count++] = '<a href="/mail/reports/sfd/'+filter.filter_id+'/"><img src="'+media_url+'mail/imgs/action_delete.png" alt="x" title="Delete" /></a>';
        rows[count++] = '</div>';
        rows[count++] = '<div class="Filter_save">';
        if(!filter.is_loaded){
            rows[count++] = '<a href="/mail/reports/sfl/'+filter.filter_id+'/"><img src="'+media_url+'mail/imgs/action_add.png" alt="Load" title="Load" /></a>';
        }else{
            rows[count++] = '<img src="'+media_url+'mail/imgs/action_add.png" alt="Load" />';
        }
        rows[count++] = '</div>';
        rows[count++] = '<div class="Filter_detail">';
    ...
    if(response.success){
        ...
        if(response.active_filters){
            ...
            if(i > 0){
                ...
                row[count++] = '<a href="/mail/reports/fd/'+i+'/"><img src="'+media_url+'mail/imgs/action_remove.png" alt="x" title="Remove" /></a>';
                row[count++] = '</div>';
                row[count++] = '<div class="Filter_save">';
                row[count++] = '<a href="/mail/reports/fs/'+i+'/"><img src="'+media_url+'mail/imgs/save.png" alt="Save" title="Save" /></a>';
                row[count++] = '</div>';
...
function addFilter(){
    ...
    $.post("/mail/reports/",add_filter_request,build_elements,"json");
    return false;
}
